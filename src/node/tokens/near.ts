...
import { NearSigner } from "@dha-team/arbundles";
...
export default class NearConfig extends BaseNodeToken {
  ...
  protected declare ARxUrl: string;

  constructor(config: TokenConfig & { ARxUrl: string }) {
  ...
  async createTx(
    ...
    if (!this.address) throw new Error("Address is undefined - you might be missing a wallet, or have not run ARx.ready()");
  ...
  async ready(): Promise<void> {
    try {
      ...
      const resolved = (await axios.get(`${this.ARxUrl}account/near/lookup?address=${base64url.encode(pubkey.split(":")[1])}`).catch((e) => {
        return e;
      })) as any;
