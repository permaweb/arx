...
import type { CreateAndUploadOptions, ARxConfig, Network, UploadResponse } from "../common/types";
...
export class BaseNodeARx extends ARx {
  ...
  constructor({
    ...
    network?: Network;
    config?: ARxConfig;
    getTokenConfig: (arx: BaseNodeARx) => NodeToken;
  }) {
    ...
    if (this.url.host.includes("devnet.arx.xyz") && !config?.providerUrl)
    ...
    this.uploader = new NodeUploader(this.api, this.utils, this.token, this.tokenConfig, this.ARxTransaction);
