...
import type { CreateAndUploadOptions, Token, ARxTransactonCtor, UploadReceipt, UploadResponse } from "../common/types";
...
export class NodeUploader extends Uploader {
  constructor(api: Api, utils: Utils, token: string, tokenConfig: Token, arxTx: ARxTransactonCtor) {
    super(api, utils, token, tokenConfig, arxTx);
  }
  /**
  ...
  public async uploadFolder(
    ...
    if (interactivePreflight) {
      if (
        !(await confirmation(
          `Authorize upload?\nTotal amount of data: ${total} bytes over ${files.length} files - cost: ${price} winc (${this.utils
            .fromAtomic(price)
