...
import type { WebARxConfig } from "../types";
...
export default class ArweaveConfig extends BaseWebToken {
  ...
  private getProvider(): Arweave {
    if (!this.providerInstance) {
      ...
      // } catch (e) {}
      this.providerInstance = (Arweave as any).init(
        /* config ??  */ {
          url: purl,
...
export class ArweaveWebARx extends BaseWebARx {
  constructor({ url, wallet, config }: WebARxConfig<Window["arweaveWallet"]>) {
    super({
      url,
      wallet,
      config,
