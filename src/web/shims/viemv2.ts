...
import { InjectedTypedEthereumSigner } from "@dha-team/arbundles/web";
      ...
      this.signer = new InjectedTypedEthereumSigner({
        getSigner: () => ({
          ...
          _signTypedData: async (domain, types, message): Promise<string> => {
            ...
            return await walletClient.signTypedData({ account: message.address, domain, types, primaryType: "ARx", message });
          },
        }),
      });
    }
