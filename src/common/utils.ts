...
export class Utils {
  ...
  public async getBalance(address: string): Promise<BigNumber> {
    const endpoint = `https://payment.ardrive.io/v1/account/balance/${this.tokenConfig.name}?address=${address}`;
    const result = await fetch(endpoint);
    if (result.ok) {
      const data = await result.json();
      return new BigNumber(data.winc);
    } else {
      if (result.status === 404) {
        return new BigNumber(0);
      }
      throw new Error(`Failed to get balance: ${result.statusText}`);
    }
  }
  ...
  /**
   * Calculates the price for [bytes] bytes paid for with [token] for the loaded ARx node.
   * @param token
   * @param bytes
   * @returns
   */
  public async getPrice(token: string, bytes: number): Promise<BigNumber> {
    const res = await fetch(`https://payment.ardrive.io/v1/price/${token}/${bytes}`);
    if (res.ok) {
      const data = await res.json();
      return new BigNumber(data.winc);
    } else {
      throw new Error(`Failed to get price: ${res.statusText}`);
    }
  }
  ...
  static async verifyReceipt(
    ...
    const dh = await dependencies.deepHash([
      dependencies.stringToBuffer("ARx"),
