...
import type { DataItem, JWKInterface } from "@dha-team/arbundles/node";
...
export class Uploader {
  ...
  protected arxTransaction: ARxTransactonCtor;

  constructor(api: Api, utils: Utils, token: string, tokenConfig: Token, arxTransaction: ARxTransactonCtor) {
    this.api = api;
    this.token = token;
    this.tokenConfig = tokenConfig;
    this.arbundles = this.tokenConfig.arx.arbundles;
    this.utils = utils;
    this.arxTransaction = arxTransaction;
  }
  ...
  public async uploadTransaction(transaction: DataItem | Readable | Buffer, opts?: UploadOptions): Promise<AxiosResponse<UploadResponse>> {
    ...
    switch (res.status) {
      ...
      default:
        if (res.status >= 400) {
          throw new Error(`whilst uploading ARx transaction: ${res.status} ${httpErrData(res)}`);
        }
    }
  ...
  /**
   * Creates & Uploads a [nested bundle] from the provided list of transactions. \
  ...
  public async uploadBundle(
    transactions: (ARxTransaction | DataItem | Buffer)[],
    opts?: UploadOptions & { throwawayKey?: JWKInterface },
  ): Promise<AxiosResponse<UploadResponse> & { throwawayKey: JWKInterface; throwawayKeyAddress: string; txs: DataItem[] }> {
