...
const ARx = new ARx("https://node1.ARx.network", "ethereum", "8da4ef21b864d2cc526dbdb2a120bd2874c36c9d0a1fb7f8c63d7f7a8b41de8f");

describe("With a constant receipt", () => {
  it("should succeed", async () => {
    await expect(ARx.verifyReceipt(receipt)).resolves.toBe(true);
  });
  it("should fail", async () => {
    expect(ARx.utils.verifyReceipt({ ...receipt, signature: "test" })).resolves.toBe(false);
  });
});
describe("With an uploaded receipt", () => {
  it("Should retrieve and verify successfully", async () => {
    const receipt = await ARx.utils.getReceipt("DQQbx79jsgI4rGlhO0arqwEMWhqWUMFTdLbO1i8M-PA");
