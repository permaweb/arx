...
export async function slam(host: string, count = 4000, min = 1_000, max = 10_000) {
  let p = [];
  const bundler = new ARx({ url: host, currency: "arweave", wallet: JSON.parse(readFileSync("./arweave.json").toString()) });
...
if (require.main === module) {
  const argF = (i) => {
    return process.argv.length - 1 >= i ? process.argv[i] : undefined;
  };
